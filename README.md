...
### 3. 特征重要性排序 (SHAP Bar Plot)
直观展示各特征对模型预测的平均绝对影响（全局宏观重要性排序）。
![SHAP特征重要性柱状图](./fig3_shap_bar.png)

### 4. 全局贡献分析 (SHAP Summary Plot)
量化各特征对预测结果的边际贡献，识别核心工艺驱动因子。
![SHAP全局解释图](./fig4_shap_summary.png)

### 5. 特征依赖分析 (SHAP Dependence Plot)
揭示核心指标（如 Sulfate）在不同浓度区间对水质判定影响的非线性趋势。
![SHAP依赖图](./fig5_shap_dependence.png)

### 6. 个体决策路径 (SHAP Waterfall Plot)
针对单一样本的判定过程进行精准拆解，实现“预测-归因”的闭环。
![SHAP瀑布图](./fig3_shap_waterfull.png)
